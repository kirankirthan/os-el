...
# Get page sequence from command-line arguments
...
pygame.display.set_caption("LFU Demand Paging Simulation")
...
# Frequency dictionary for LFU
frequency = defaultdict(int)

# Main loop
def simulation():
    global frames, page_faults, frequency
    ...
    while current_page_index < len(page_sequence):
        ...
        if page not in frames:
            ...
            if None in frames:
                ...
                # Use LFU to determine which page to replace
                least_frequent = min(frames, key=lambda x: frequency[x])
                lfu_index = frames.index(least_frequent)
